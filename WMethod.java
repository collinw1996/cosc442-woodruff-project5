...
/**
 * 
 * Program to compute the W-set for a given FSM. Based on Chow's algorithm
 * published in 1980 This program is based on some code written by Aditya for
 * the UIO algorithm.
 * 
 ...
 * 
 * Originally coded by: Aditya Mathur (for the UIO algorothm. Some of those
 * files are used here). W-method and test sets coded by a bright Purdue
 * undergraduate students.
 * 
 * Comments:Variables in the initial version are the same as given by the
 * authors of the algorithm. Java naming norms may be violated.
 * 
 * Updated: August 1, 2013 Aditya Mathur
 * 
 * Several classes and methods used here have been coded by Brandon Wuest while
 * he was an undergraduate student at Purdue.
 * 
 */
...
public class WMethod {
	...
	/**
	 * 
	 * Method to read the FSM description from a source file. startState is assumed
	 * to be 1.
	 */
	...
	public static void getFSM() {
		...
		try {
			...
			Utilities.printException("wAlgoriithm", "getFSM",
					"IO exception raised while reading: " + fsmFilename + " not found");
	...
	static void splitTransition(String transition) {
		...
		/*
		 * Split a line of input from the FSM file. Example: Input line: 3 4 a/0 Note
		 * the three tokens 3, 4, and a/0. Indicates: Source state: 3 Destination state:
		 * 4 Take the transition on input: a Generate output: 0.
		 */
		...
		// Split transition into its three components: source, destination and edge
		// label.
		...
		try {
			...
			Utilities.printException("WMethod", "splitTransition",
					"From state ID must be a number, found: " + token[0]);
		}
	...
	public static Vector<String> generateTests(TestingTree tree, pTableManager tablemanager) {
		...
		// We assume m=n, where m is the number of states in the FSM of the program
		// under test
